...
const PATTERNS: Array<{ type: IntentType; patterns: RegExp[] }> = [
  ...
  },
  {
    ...
    patterns: [
      ...
      /faucet/i,
      /request\s+(?:testnet\s+)?(?:eth|funds|tokens)/i,
      /testnet\s+(?:eth|funds|tokens)/i,
      /get\s+me\s+(?:some\s+)?(?:eth|tokens|funds)/i,
      /where\s+(?:can|do)\s+(?:i|we)\s+get\s+(?:testnet\s+)?(?:tokens|funds|eth)/i,
      /how\s+(?:can|do|to)\s+(?:i|we)\s+get\s+(?:testnet\s+)?(?:tokens|funds|eth)/i,
